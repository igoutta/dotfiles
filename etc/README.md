...
| `systemd/system/ghostmirror*.{service,timer}` | `/etc/systemd/system/` | Espejos de pacman en dos pasos: mensual elige 40 espejos https por sanidad y latencia entre Worldwide, EE. UU., Brasil, Chile, Colombia, México y Alemania; semanal vuelve a medir la velocidad de esos 40. Units propios (el paquete no trae), revisados el 2026-09-25. Solo se habilitan los timers. |
...
| `sudoers.d/10-wheel` | `/etc/sudoers.d/10-wheel` | sudo para el grupo wheel, sin tocar `/etc/sudoers`. Modo 440, validar con `visudo -c`. Ubuntu: grupo `sudo`; Fedora: ya viene activo. |
| `sudoers.d/20-defaults` | `/etc/sudoers.d/20-defaults` | Cinco intentos, sin sermón, con burlas al fallar, asteriscos al teclear, contraseña válida 10 min para todas las terminales, `sudoedit` con Helix. Modo 440, `visudo -c`. |
| `security/faillock.conf` | `/etc/security/faillock.conf` | Bloqueo por contraseñas fallidas (sudo, greeter, bloqueo de pantalla): 5 fallos, 2 min. Mismo archivo en Ubuntu/Fedora. |
